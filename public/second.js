  ...
  function nextQuestion(){
    ...
    for(var i=0;i<options.length;i++){
        if(options[i].checked){
            ...
            var getOptions=questionsCSS[index-1][`option${userSelectedInput}`];
            // console.log(getOptions);
            var correctAnswer=questionsCSS[index-1]['corrAnswer'];
    ...
    // ending the quiz
    if(index>questionsCSS.length-1){
        // console.log(score);
        var percentage=(score/questionsCSS.length)*100;
        if (percentage>=80){
        Swal.fire({
            title: "Good job!",
    ...
    questionElement.innerText=questionsCSS[index].question;
    opt1.innerText=questionsCSS[index].option1;
    opt2.innerText=questionsCSS[index].option2;
    opt3.innerText=questionsCSS[index].option3;
    index++;
    }
